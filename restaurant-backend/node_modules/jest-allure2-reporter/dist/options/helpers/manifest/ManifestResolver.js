...
class ManifestResolver {
    ...
    extract = async (maybePackageName, maybeExtractor, maybeDefaultValue) => {
        ...
        if (!manifestPath) {
            logger_1.log.warn(`Cannot find package.json for ${what}`);
            return defaultValue;
        }
        try {
            ...
            logger_1.log.error(error, `Failed to extract package.json for ${what}`);
            return;
        }
    };
    ...
    resolveCJS(packageName) {
        try {
            ...
            logger_1.log.debug(error, `Failed to resolve package.json for "${packageName}"`);
            return;
        }
    }
}
